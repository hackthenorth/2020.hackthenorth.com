...
const CarouselTrack = styled(Flex)<CarouselTrackProps>`
  position: relative;
  width: max-content;
...
const InfiniteCarousel: React.FC<CarouselProps> = ({
  ...
  const { width: originalElementsWidth } = useDimensions(elementsRef, {
  ...
  const { onHoverStart, onHoverEnd, toggleHover } = useHoverEvents(
    setIsHovering
  );
  ...
  const shouldStop = useMemo(() => (isHovering && stopOnHover) || !isInView, [
    stopOnHover,
    isHovering,
    isInView,
  ]);
